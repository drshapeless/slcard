...
void updatePosition(struct slPosition *positions, struct slWindow windowPara);
void drawFlashcard(struct FlashcardGame *game, struct slWindow windowPara,
		   struct slPosition positions, TTF_Font *font,
		   SDL_Renderer *renderer);
SDL_Texture *createUtf8StringTexture(char *string, TTF_Font *font,
				     SDL_Color color, SDL_Renderer *renderer);
void windowResizeCallback(SDL_Window *window, struct slWindow *windowPara,
			  struct slPosition *positions);
void updateFont(TTF_Font *font, int fontsize);
void processKeyboardInput(SDL_Keycode keycode, struct FlashcardGame *game,
			  struct slWindow *windowPara,
			  struct slPosition *positions, TTF_Font *font,
			  int *exit);
...
void guiRun(struct FlashcardGame *game)
{
	...
	SDL_Window *window = SDL_CreateWindow(
		WINDOW_TITLE, 0, 0, WINDOW_WIDTH, WINDOW_HEIGHT,
	...
	struct slWindow windowPara = { .width = WINDOW_WIDTH,
				       .height = WINDOW_HEIGHT,
				       .padding = WINDOW_PADDING,
				       .fontsize = FONT_SIZE };

	struct slPosition *positions = malloc(sizeof(struct slPosition));
			...
			case SDL_WINDOWEVENT:
				if (e.window.event == SDL_WINDOWEVENT_RESIZED) {
					windowResizeCallback(
						window, &windowPara, positions);
					drawFlashcard(game, windowPara,
						      *positions, font,
						      renderer);
				}
				break;
			case SDL_KEYDOWN:
				processKeyboardInput(e.key.keysym.sym, game,
						     &windowPara, positions,
						     font, &exit);
				drawFlashcard(game, windowPara, *positions,
					      font, renderer);
				break;
			}
		}
...
void updatePosition(struct slPosition *positions, struct slWindow windowPara)
{
	...
	positions->counterRect =
		createRect(width - padding - fontsize * 4, padding, 0, 0);
...
void drawSmallText(SDL_Renderer *renderer, char *string, TTF_Font *font,
		   SDL_Color color, SDL_Rect rect)
{
	int w, h;
	SDL_Texture *texture =
		createUtf8StringTexture(string, font, color, renderer);
...
SDL_Texture *createUtf8StringTextureWrapped(char *string, TTF_Font *font,
					    SDL_Color color,
					    SDL_Renderer *renderer,
					    int renderWidth)
{
	SDL_Surface *surface = TTF_RenderUTF8_Blended_Wrapped(
		font, string, color, renderWidth);
	if (!surface) {
...
char *breakString(char *string, TTF_Font *font, SDL_Renderer *renderer,
		  int width)
{
...
void drawText(SDL_Renderer *renderer, char *string, TTF_Font *font,
	      SDL_Color color, SDL_Rect rect, int width)
{
	int w, h;
	...
	for (int i = 0; i < stringArrPos; i++) {
		texture = createUtf8StringTexture(stringArr[i], font, color,
						  renderer);
		SDL_QueryTexture(texture, NULL, NULL, &w, &h);
		rect.w = w;
		rect.h = h;
		   ...
		   SDL_Renderer *renderer)
{
	...
	if (game->cardIndexes) {
		...
		drawSmallText(renderer, memoryString, font, yellow,
			      positions.memoryRect);

		/* Draw scores. */
		char badScoreString[COUNT_SIZE];
		sprintf(badScoreString, "%i", game->badScore);
		SDL_Color red = { 255, 0, 0, 255 };
		drawSmallText(renderer, badScoreString, font, red,
			      positions.badScoreRect);

		char goodScoreString[COUNT_SIZE];
		sprintf(goodScoreString, "%i", game->goodScore);
		SDL_Color green = { 0, 255, 0, 255 };
		drawSmallText(renderer, goodScoreString, font, green,
			      positions.goodScoreRect);

		/* Draw counter. */
		char counterString[COUNT_SIZE * 3];
		sprintf(counterString, "%d / %d", game->pos + 1,
			game->gameSize);
		SDL_Color white = { 255, 255, 255, 255 };
		drawSmallText(renderer, counterString, font, white,
			      positions.counterRect);

		/* Draw question. */
		drawText(renderer, currentQuestion(game), font, white,
			 positions.questionRect,
			 windowPara.width - windowPara.padding * 2);

		/* Draw answer. */
		if (game->showAnswer) {
			drawText(renderer, currentAnswer(game), font, white,
				 positions.answerRect,
				 windowPara.width - windowPara.padding * 2);
		}
	} else {
		SDL_Color white = { 255, 255, 255, 255 };
		char *endGameMessage0 = "You have completed all cards.";
		char *endGameMessage1 =
			"Press T to add one memory count to all cards. Press ESC to close slcard.";
		drawText(renderer, endGameMessage0, font, white,
			 positions.questionRect,
			 windowPara.width - windowPara.padding * 2);
		drawText(renderer, endGameMessage1, font, white,
			 positions.answerRect,
			 windowPara.width - windowPara.padding * 2);
	}
